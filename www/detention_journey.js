...
let _detentionAnimationFrameId;
let detentionCanvas;
let detentionCtx;
let _currentHighlight = null; // 'none', 'person1', 'person2', 'chicago', 'facility1', 'facility2'
...
const _HIGHLIGHT_PARTICLE_RADIUS = 5;
const BACKGROUND_PARTICLE_COLOR = "#88888850"; // Darker gray, more visible
const _HIGHLIGHT_PERSON1_COLOR = "#ff0000ee"; // Red, more opaque
const _HIGHLIGHT_PERSON2_COLOR = "#0066ffee"; // Blue, more opaque
...
async function loadDetentionData() {
    try {
        ...
        console.log(
            "Top person has",
            detentionHighlights[0]?.unique_facilities,
            "unique facilities"
        );
        console.log(
            "Average transfer distance:",
            detentionStatistics.overall_stats.avg_transfer_distance_miles,
            "miles"
        );

        return {
            flows: detentionFlows,
            highlights: detentionHighlights,
            statistics: detentionStatistics,
        };
    } catch (error) {
...
function createDetentionParticles(flows, _highlights) {
    const particles = [];
    ...
    flows.forEach((flow, flowIndex) => {
        ...
        const line = turf.greatCircle(turf.point(startCoords), turf.point(endCoords), {
            npoints: 50,
        });
    ...
    ["person1", "person2"].forEach((personKey, personIndex) => {
        ...
        if (point.trail.length > 1) {
            for (let i = 0; i < point.trail.length - 1; i++) {
                ...
                const trailAlpha = Math.floor(trailOpacity * 255)
                    .toString(16)
                    .padStart(2, "0");
        ...
        const glowAlpha1 = Math.floor(100 * pulseIntensity)
            .toString(16)
            .padStart(2, "0");
        const glowAlpha2 = Math.floor(40 * pulseIntensity)
            .toString(16)
            .padStart(2, "0");
        ...
        const mainAlpha = Math.floor(220 * pulseIntensity)
            .toString(16)
            .padStart(2, "0");
        detentionCtx.beginPath();
...
function drawDetentionParticles() {
    ...
    _detentionAnimationFrameId = requestAnimationFrame(drawDetentionParticles);
}
...
function createDetentionCanvasOverlay() {
    ...
    console.log(
        "Detention canvas overlay created:",
        detentionCanvas.width,
        "x",
        detentionCanvas.height
    );
}
...
function populateDetentionStoryCards(highlights) {
    highlights.forEach((person, index) => {
        ...
        if (journeyEl && story) {
            const firstDate = story.first_date
                ? new Date(story.first_date).toLocaleDateString()
                : "Unknown";
            const lastDate = story.last_date
                ? new Date(story.last_date).toLocaleDateString()
                : "Unknown";
    ...
    scroller
        ...
        .onStepEnter((response) => {
            ...
            if (stepClass.includes("intro-step")) {
                ...
                _currentHighlight = "chicago";
                animatedPoints.person1.visible = false;
                animatedPoints.person2.visible = false;
                if (detentionStatistics?.featured_locations[0]) {
                ...
                _currentHighlight = "facility1";
                animatedPoints.person1.visible = false;
                animatedPoints.person2.visible = false;
                if (detentionStatistics?.featured_locations[1]) {
                ...
                _currentHighlight = "facility2";
                animatedPoints.person1.visible = false;
                animatedPoints.person2.visible = false;
                if (detentionStatistics?.featured_locations[2]) {
        ...
        .onStepExit((response) => {
            ...
            if (stepClass.includes("interactive-step") && response.direction === "down") {
                ...
                setTimeout(() => {
                    ...
                    // Fade out cards
                    document
                        .querySelectorAll("#detention-section article .step")
                        .forEach((step) => {
