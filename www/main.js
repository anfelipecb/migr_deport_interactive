...
const _PARTICLE_COLOR = "#FF000080";
...
let topDestinations = [];
let _allFlows = []; // Store all flows for country aggregation
...
function drawParticles() {
    ...
    _animationFrameId = requestAnimationFrame(drawParticles);
}
...
async function loadWorldCountries() {
    try {
                ...
                if (isRussia) {
                    ...
                    if (feature.geometry.type === "MultiPolygon") {
                        ...
                        feature.geometry.coordinates = feature.geometry.coordinates.filter(
                            (polygon) => polygon && polygon.length > 0
                        );
                    }
                }
            });
        ...
        const russiaFeature = geojson.features?.find(
            (f) =>
        ...
        if (russiaFeature) {
            console.log(
                "Russia found:",
                russiaFeature.properties?.name || russiaFeature.properties?.NAME,
                "Type:",
                russiaFeature.geometry?.type
            );
        }
...
function getDestinationDataForCountry(countryName) {
    ...
    for (const destName in countryRemovalData) {
        ...
        if (
            normalizedDest.includes(normalizedCountry) ||
            normalizedCountry.includes(normalizedDest)
        ) {
            return countryRemovalData[destName];
        }
    }
...
function setupCountryInteractions() {
    ...
    map.on("mousemove", COUNTRIES_LAYER_ID, (e) => {
        ...
        map.setFeatureState({ source: COUNTRIES_SOURCE_ID, id: countryName }, { hover: true });
...
function setCountryLayersVisible(visible) {
    ...
    console.log(
        `Country layers visibility set to: ${visibility}, countriesActive: ${countriesActive}`
    );
}
...
function initMap() {
    ...
    map.on("load", async () => {
        ...
        _allFlows = flows; // Store for country aggregation
        ...
        console.log(
            `Aggregated removal data for ${Object.keys(countryRemovalData).length} destination countries`
        );
        ...
        if (worldCountriesData) {
            ...
            console.log(
                "Country layers added. Layer exists:",
                map.getLayer(COUNTRIES_LAYER_ID) !== undefined
            );
        } else {
    ...
    switch (response.index) {
        ...
        case 4:
            ...
            setTimeout(() => {
                ...
                console.log("Layer exists:", map?.getLayer(COUNTRIES_LAYER_ID));
            }, 100);
