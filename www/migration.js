...
function createMigrationVisualization() {
    ...
    const icons = iconGroup.selectAll('.person-icon')
        .data(positions)
        .enter()
        .append('g')
    ...
    const personGroup = icons.append('g')
        .attr('transform', `translate(-${PERSON_SIZE/2}, -${PERSON_SIZE/2}) scale(${PERSON_SIZE/24})`);
    
    personGroup.append('path')
        .attr('d', PERSON_SVG_PATH)
        .style('fill', '#ccc');
...
function updateMigrationViz(year) {
    ...
    const previousIconCount = previousData.length > 0 
        ? Math.max(...previousData.map(d => d.iconCount))
        : 0;
    
    console.log(`Year ${year}: showing ${iconCount} icons (${previousIconCount} previous + ${iconCount - previousIconCount} new)`);
    ...
    loadMigrationData().then(data => {
        ...
        scroller
            ...
            .onStepEnter(response => {
                ...
                const year = parseInt(response.element.dataset.year);
        ...
        window.addEventListener('resize', () => {
            ...
            if (activeStep) {
                const year = parseInt(activeStep.dataset.year);
                updateMigrationViz(year);
            }
        });
