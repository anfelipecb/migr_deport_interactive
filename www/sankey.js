...
const sankey = d3.sankey()
    ...
    const link = g.append('g')
        ...
        .on('mouseover', function(event, d) {
            ...
            tooltip.transition()
                .duration(200)
                .style('opacity', 1);
            tooltip.html(`<strong>${sourceName} → ${targetName}</strong><br/>${d.value.toLocaleString()} people`)
    ...
    node.append('rect')
        ...
        .on('mouseover', function(event, d) {
            ...
            tooltip.transition()
                .duration(200)
                .style('opacity', 1);
            tooltip.html(`<strong>${nodeName}</strong><br/>Incoming: ${incoming.toLocaleString()}<br/>Outgoing: ${outgoing.toLocaleString()}`)
                .style('left', (event.pageX + 15) + 'px')
                .style('top', (event.pageY - 28) + 'px');
        })
        .on('mousemove', function(event, d) {
    ...
    node.append('text')
        .attr('x', (d) => {
            ...
            console.log(`Label for ${d.name}: depth=${d.depth}, y0=${d.y0.toFixed(1)}, y1=${d.y1.toFixed(1)}, height=${height.toFixed(1)}, localY=${localY.toFixed(1)}, labelX=${x}`);
            return x;
        })
