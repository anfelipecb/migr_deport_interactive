...
let timelineG = null;
let _trumpMarkerLine = null;
let _trumpMarkerLabel = null;
...
async function initTimeline() {
    try {
        ...
        if (!responseAll.ok || !responseByCountry.ok) {
            throw new Error(
                `HTTP error! status: ${responseAll.status} or ${responseByCountry.status}`
            );
        }
        ...
        if (!timelineDataAll.data || timelineDataAll.data.length === 0) {
            ...
            document.getElementById("timeline-viz").innerHTML = "<p>Error: No data available.</p>";
            return;
        }
...
function populateCountryFilter() {
    ...
    // Add country options
    if (timelineDataByCountry?.countries) {
...
function renderTimeline(country = "all") {
    ...
    if (country === "all") {
        ...
        if (timelineDataByCountry?.data[country]) {
            data = {
                data: timelineDataByCountry.data[country],
                dateRange: timelineDataAll.dateRange,
                totals: {
                    arrests: timelineDataByCountry.data[country].reduce(
                        (sum, d) => sum + d.arrests,
                        0
                    ),
                    detentions: timelineDataByCountry.data[country].reduce(
                        (sum, d) => sum + d.detentions,
                        0
                    ),
                    removals: timelineDataByCountry.data[country].reduce(
                        (sum, d) => sum + d.removals,
                        0
                    ),
                },
            };
    ...
    _trumpMarkerLine = null;
    _trumpMarkerLabel = null;
    ...
    const maxValue = d3.max(dataParsed, (d) => Math.max(d.arrests, d.detentions, d.removals));
    ...
    timelineG.append("g").attr("class", "y-axis").call(yAxis);
    ...
    const detentionsLine = timelineG
        .append("path")
    ...
    // Create legend
    const legend = timelineSvg
        .append("g")
        .attr("class", "timeline-legend")
        .attr(
            "transform",
            `translate(${timelineWidth - timelineMargin.right - 150}, ${timelineMargin.top + 20})`
        );
...
function drawTrumpMarker() {
    ...
    const _yScale = window.timelineYScale;
